...
int main() {
  ...
  for (const auto &shape :
       {std::pair{128, 1024}, {200, 4096}, {256, 4096}, {4096, 7168}}) {
    int M = shape.first;
    int K = shape.second;
    size_t n = (size_t)M * K;
    ...
    for (size_t i = 0; i < gd.size(); i++)
      bad += gd[i] != rd[i];
    for (size_t i = 0; i < gsf.size(); i++)
      bad += gsf[i] != rsf[i];

    float ms = time_avg_ms([&] { launch_nvfp4_quant(dx, dd, dsf, M, K, sms); },
                           M >= 4096 ? 50 : 200);
    double bytes = n * 2.0 + n * 0.5 + n / 16.0;
    printf("M=%-5d K=%-5d  %s(bad=%ld)  %8.2f us  %6.0f GB/s\n", M, K,
           bad ? "FAIL" : "PASS", bad, ms * 1e3, effective_gbps(bytes, ms));
    total_bad += bad;
    cudaFree(dx);
    cudaFree(dd);
    cudaFree(dsf);
