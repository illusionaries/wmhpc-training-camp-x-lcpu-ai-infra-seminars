...
__global__ void tile_kernel(const __nv_bfloat16 *gA, const __nv_bfloat16 *gB,
  ...
  if (warp == 0) {
    ...
    if constexpr (GROUP == 1) {
      ...
      asm volatile("tcgen05.alloc.cta_group::2.sync.aligned.shared::cta.b32 "
                   "[%0], %1;" ::"r"(dst),
                   "r"(64));
      asm volatile(
  ...
  if (warp == 0 && elected && (GROUP == 1 || rank == 0)) {
    ...
    if constexpr (GROUP == 1) {
      ...
      asm volatile(
          ...
          ".shared::cluster.multicast::cluster.b64 [%0], %1;" ::"r"(mbar_u32),
          "h"((uint16_t)0x3)
          : "memory");
    }
  }
...
int main() {
  ...
  float t2 = time_avg_ms(
      ...
      200);
  printf("::1  %s(bad=%ld)  %.2f us\n", bad1 ? "FAIL" : "PASS", bad1, t1 * 1e3);
  printf("::2  %s(bad=%ld)  %.2f us\n", bad2 ? "FAIL" : "PASS", bad2, t2 * 1e3);
  return (bad1 || bad2) != 0;
}
