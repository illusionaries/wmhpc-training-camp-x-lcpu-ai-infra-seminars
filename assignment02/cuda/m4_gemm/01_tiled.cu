...
__global__ void gemm_tiled(const __nv_bfloat16 *gA, const __nv_bfloat16 *gB,
  ...
  auto mbar_shared_addr = __cvta_generic_to_shared(&mbar);
  auto tmem_addr_shared_addr = __cvta_generic_to_shared(&tmem_addr);

  if (warp_id == 0) {
    if (lane_id == 0) {
      asm volatile(
          "mbarrier.init.shared::cta.b64 [%0], %1;" ::"l"(mbar_shared_addr),
          "r"(1));
      asm volatile(
          "fence.mbarrier_init.release.cluster;"); // mem access ordering
    }
  ...
  for (int it = 0; it < K / BK; it++) {
    ...
    for (int i = threadIdx.x; i < BM * BK; i += blockDim.x) {
      auto row = i / BK;
      auto col = i % BK;
      *reinterpret_cast<__nv_bfloat16 *>(&sA[swz128(row, col * 2)]) =
          gA[(tileM + row) * K + it * BK + col];
    }
    for (int i = threadIdx.x; i < BK * BN; i += blockDim.x) {
      auto row = i / BK;
      auto col = i % BK;
      *reinterpret_cast<__nv_bfloat16 *>(&sB[swz128(row, col * 2)]) =
          gB[(tileN + row) * K + it * BK + col];
    }
...
int main(int argc, char **argv) {
  ...
  CUDA_CHECK(cudaFuncSetAttribute(
      gemm_tiled, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smemBytes));
  auto launch = [&] {
...
// [4.1 tiled] M=4096 N=4096 K=4096  PASS(bad=0)  3.39 ms  40.6 TFLOPS  (cuBLAS 1764.6, 达成率 2%)